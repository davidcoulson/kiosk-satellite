...
* **Hold duration** under Settings > Gestures > Hand Gestures controls how long the same finger gesture must stay detected before its action runs. Choose Instant or 0.5 to 3 seconds in half-second steps. The default is 1 second to reduce triggers from passing hands. Choose Instant to trigger an action on the first matching reading. The same setting is available in Remote Admin. Changing the finger count or removing the hand cancels the hold. Brief tracking gaps are tolerated, but an action can only fire on a fresh matching reading.
