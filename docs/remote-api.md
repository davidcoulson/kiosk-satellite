...
| `/api/health` | GET | The Device Info tab's Hardware section as one JSON object: identity, addresses, battery (null on a device without one), screen (size, density, `orientation` and `rotation` in degrees), the system `webview` (package and version), the network `link` (`type`: ethernet/wifi/cellular/vpn/other, plus `rssi` and `speedMbps` on Wi-Fi), RAM, storage, CPU usage and temperature, and uptimes (`uptime.app`, `uptime.device` and `uptime.network`, seconds; `network` is null while offline and starts counting at app start at the earliest). Meant for external monitoring to poll, so it is the one endpoint that needs no token |
