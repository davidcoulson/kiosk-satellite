...
export function formatPluginReading(reading) {
  ...
  if (state == null) return { value: pluginText('No data'), unit: '' };
  if (typeof state === 'boolean') {
    return { value: state ? pluginText('On') : pluginText('Off'), unit: '' };
  }
  if (typeof state === 'number') {
    if (!Number.isFinite(state)) return { value: pluginText('No data'), unit: '' };
  ...
  return { value: state === '' ? pluginText('Empty') : String(state), unit: '' };
}
