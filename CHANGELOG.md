...
## v2026.9.81-djc - 2026-09-24

### Added
- **Agent mode.** Runs an install as a management agent instead of a kiosk, for a projector, a media box or anything else that is not a wall panel: the device still appears in Home Assistant with its sensors over ESPHome, still answers the remote admin, still takes updates and still belongs to the fleet, but the browser, kiosk lock, launchers, screensavers, theater mode, cameras, motion, voice, media players and the intercom never start, and the page bridge is never built. The app shows a status card with its version and admin address instead of a dashboard. A mode rather than a second build, so there is still one APK to make and install. Settings → Device → User Interface → Agent mode; takes a restart. An agent's admin is trimmed to what it runs: the header carries an **Agent** tag, and the Home Assistant Setup, Voice Satellite, Screensaver, Web Browsing, Media Player, DLNA, Intercom, Camera, Camera Streams, Kiosk Mode, Lockdown, Home Launcher and Gestures pages go, along with the Reload page, Clear cache, Screensaver, Camera view, Postpone, Do not disturb, Take snapshot and Exit app quick controls and the Now playing card - every one of which sends a command an agent does not register. ESPHome, Screen & Audio, App Launcher, Device, Fleet, Files, Plugins, Logs and About stay. An agent also stops publishing the **Screenshot** camera, its Take screenshot button and its Last screenshot timestamp over ESPHome — the same picture of someone else's UI, through the other door. An agent's admin no longer shows the screen card either — capturing and encoding a projector's own screen every five seconds, to paint a picture of something the kiosk does not draw, is work for nothing — and it never takes the one-shot capture at load. An agent's admin stops reporting the absence of things it does not run: the "Home Assistant not set up" attention card, the Voice Satellite and Media Player status tiles, and the settings pages for Browser, Camera, Screensaver, Sendspin, Voice Satellite, Kiosk, Intercom, DLNA, Gestures, Lockdown, Web Content and Home Assistant are all hidden there - Screen & Audio, Device, ESPHome, Fleet and Launcher stay, because those are what an agent is. A kiosk in agent mode also announces itself as one over mDNS, so Switch kiosk shows an **Agent** tag beside it. The remote admin no longer treats an agent as unconfigured: it decided that from an empty Start page, which an agent never has, so logging into a projector's admin asked for a Home Assistant URL and token for a dashboard it will never show. An agent keeps the app launcher (for `installedApps`, `foregroundApp` and the foreground_app sensor) and registers `launchApp` and `openUri` itself, so Home Assistant can start Plezy, Kodi or a deep link on a projector through the ESPHome `launch_app` action that already existed. The renderer watchdog knows about the mode too: it reads "foregrounded with no WebView" as a wedged renderer, which is an agent's normal state, so it never arms there.
...
### Fixed
- **An update or a restart no longer switches every plugin off.** Plugins are switched off after a startup that never finished, in case one of them is taking the app down. But the check could not tell a plugin crash from an app update or a Restart app landing inside the first 30 seconds, so a quick update-then-restart left every plugin disabled and Home Assistant without their entities. An app update in between and a deliberate restart or exit no longer count, it now takes two incomplete startups in a row, and plugins the check switched off come back by themselves with the next update (including ones switched off by earlier builds).
...
### Added
- **Show seconds for the Weather Mood clock.** The Weather Mood clock gets the Show seconds toggle the Clock screensaver has. Pixel shift still moves the clock once a minute (#710).
...
### Changed
- **The Microphone level bar works without Voice Satellite.** The bar in Microphone settings and its remote admin twin read the capture directly instead of the wake word engine, so they move before Voice Satellite has started one. Tied to the engine, the bar stayed empty on a device where Voice Satellite had not started and looked exactly like a dead microphone. It still only appears with wake word detection on.
...
### Fixed
- **A microphone that delivers nothing no longer leaves the device silently deaf.** When the audio system could not read the microphone at all, which happens with a mono USB microphone on a ROM that opens USB input as stereo, the capture waited forever for audio that never came. None of the capture checks ran and nothing reached the log. A read that delivers nothing for three seconds now counts as a failed format: capture tries the other formats, backs off when none works and says so in the app log, naming the microphone and, for USB, the likely cause.
...
### Fixed
- **Portal Go camera images reach Home Assistant.** Camera snapshots now omit unused bytes after the JPEG end marker. The Portal Go could append more than 12 MB of padding to a small image, causing Home Assistant to discard it for exceeding its image size limit. The original image quality and metadata are preserved.
...
### Fixed
- **ESPHome camera previews keep entities online.** Large camera images now send in chunks as the connection drains instead of filling the outgoing message queue and disconnecting Home Assistant. Opening a camera preview or refreshing the device page no longer interrupts sensor updates on affected devices such as the Portal Go.
...
### Fixed
- **Fleet members stay listed without mDNS advertisements.** The remote admin kiosk switcher and intercom use saved membership alongside network discovery. Leaders share the member list with followers and poll saved addresses even when multicast is unavailable. Intercom marks failed probes as Unreachable and checks members again when they return. Device and remote admin guidance and the address-mismatch error include English and Spanish text (#596).
- **Background listening updates without refreshing remote settings.** Toggling **Keep listening in the background** shows or hides **Return to the previous app** in place and keeps it directly below the parent switch. Live settings updates preserve the existing controls instead of rebuilding every tab (#593).
- **Scheduled switches from Immich to the clock keep updating.** Releasing the photo metadata corners during the switch could freeze the displayed frame and leave taps without a visible response. Corner cleanup now runs after widget disposal so the clock and dashboard keep rendering (#592).
- **Remote administration refreshes every JavaScript module after an upgrade.** Version stamping now includes side-effect and dynamic imports so cached settings modules cannot mix with the new build.
...
- **Hand Gesture Tester.** Settings > Gestures on the device has a new group that opens a live tester for the Show fingers gesture. A hand figure lights up each finger as the camera reads it raised, with the count, the gesture that count would trigger and how to hold the hand. Gestures do not fire while the tester is open, and it runs the camera on its own, so it works before any hand gesture is mapped.

### Fixed
- **Show an open hand now fires.** The hand tracker never counted the thumb, so a fully open hand read as four fingers and a mapping set to Open hand (5) could never fire. The thumb now counts once all four fingers are up, when it is spread out from the palm rather than tucked across it. An open hand reads as five and four fingers with the thumb folded across the palm still read as four. Hands showing one to three fingers keep ignoring the thumb, so a thumb resting beside two fingers still does not read as three (#581).
- **Kiosks that start before their network is up now find the others once it is.** A kiosk whose app came up with no network, a Portal after a reboot among them, joined the mDNS group on whatever interface existed at that moment and kept listening there once Wi-Fi arrived, so the kiosk switcher and the intercom listed nobody until the app was restarted. Discovery now joins the group on each network interface and joins again when the network changes and at every announcement, so the others show up as soon as the network comes up (#582).

### Changed
- **Remote administration follows device changes over WebSocket subscriptions.** Settings edited on the kiosk update the remote admin automatically. Commands and settings saves share the same connection. Open device settings pages also reflect remote saves immediately. Visiting Cameras or Gestures keeps other remote controls connected to the shared settings state. Custom controls follow changes too, including Media Player selections, Sonos speakers, audio devices and plugin settings. Visible panels subscribe to their live state, native diagnostics share observers and disconnected clients recover current state when they reconnect. The page opens the socket before it builds itself and reads the device info, settings, console and log history through it, so loading the remote admin no longer issues a burst of HTTP reads and then repeats them once the socket connects. The device's Sonos speakers list follows speakers added or forgotten from the remote admin. A Bluetooth proxy relaying beacons no longer counts as a status change, so the Overview stops re-reading its health row every five seconds while the proxy is busy. A settings write is not a status change either: each manager announces its own status when it moves and the Overview re-reads only that source, so flipping a switch no longer sends ten status commands, and the service and Bluetooth samples arrive with the change so the page paints without asking. Section URLs use readable slugs such as `#camera/rtsp-onvif-streaming` while older bookmarks still work.
- **The remote admin covers itself while the connection is away.** When the socket to the kiosk drops, a Reconnecting notice blocks the page until it is back, so nothing is changed against a kiosk that cannot hear it. After twenty seconds the notice offers a reload.
- **The remote admin reloads itself after an update.** When the app restarts on a new build and the connection returns, the page shows which version the device is now running and reloads five seconds later, or right away from the button, instead of leaving the previous version's page on screen.
- **Camera streaming port descriptions are shorter.** The RTSP and ONVIF port fields now show only the server port description.
...
### Fixed
- **Links on the About page open in the link overlay.** Tapping the GitHub, website or license link under Settings > About loaded the page into the dashboard view itself, which replaced the dashboard and took the Voice Satellite session down with it. Those links, and the Voice Satellite link on its own page, now open above the dashboard with a close button, the way a tapped dashboard link does.
- **The dashboard update filter stands down on a view that uses most of the instance.** A wall of area cards subscribes to every entity in the house, so the filter had nothing to drop, and the tracking that finds a view's entities charged every state read on every re-render. On a dashboard of eight area cards that doubled the WebView renderer's CPU, 63% of a core against 30% with the filter off, for updates that all went through anyway. Once a view has read more than half of the instance's entities the filter now switches itself off for that view, hands the cards the plain state table again and says so in the remote admin, on the Overview tile and in the filter row under Browser: "This view uses 3902 entities, which crosses the filtering threshold. Filtering is disabled." Another view starts fresh, and a view the filter helps is not touched.
...
### Added
- **Detection can bring the screensaver back instead of the dashboard.** A new Wake to screensaver switch under Turn screen off after changes what motion, face, proximity and person detection do once the panel has powered off: with it on, a detection only turns the screen back on, the screensaver is still up and the screen-off countdown starts over, the same way the ESPHome Screen light has always woken a kiosk. A touch is what opens the dashboard. This is the photo frame pattern Fully Kiosk users asked for: someone walking into the room brings the photos back and a quiet room lets the panel go dark again. Detection on a lit screensaver keeps its rules, so pair it with Only when screen is off for a slideshow that survives people moving about. The wake word and the ESPHome dismiss still land on the dashboard.
- **An APK uploaded through the remote admin installs the kiosk.** For a kiosk that can reach neither GitHub nor a custom repository, Settings > Device > Updates gains an Install from file row in the remote admin: pick a Kiosk Satellite APK on the computer, the browser uploads it with a progress count, the kiosk reads package, version and build out of the file, and a confirmation names what arrived and what the kiosk runs before the same installer the GitHub download uses takes it, silent or confirmed on the kiosk screen as ever. The kiosk refuses another app's package, an older build (Android would not install one either) and a file its cache cannot hold twice, and says why. A fleet leader gets an Install on the fleet choice on the same confirmation: the uploaded APK streams to each online follower, which checks it under the same rules and installs it, and the leader installs its own copy last. Behind it are the `POST /api/update/upload` endpoint, which takes the APK as the raw body, and the `installUploadedApk` and `fleetInstallUploaded` commands, all open to a fleet token too.
- **An Updates only fleet profile.** A second built-in profile next to the Default: nothing syncs to a kiosk on it, so it keeps every setting of its own and only gets updates pushed by the leader. It cannot be changed or deleted, but it can be duplicated into a profile of your own.
...
- **A return without the input focus counts as a return.** Android reports the app back in front only once its window also holds the input focus, and a device with a focus-holding window over the kiosk never reports it. Everything that waited for that report stayed stuck with the kiosk in plain view: the screensaver kept its idle clock on hold and refused Now Playing on playback, so only the floating player showed; the wake word pulled the app forward on every detection; the app launcher's auto-return fired on a kiosk that was already back; dashboard rotation froze; the Home Assistant socket got no reconnect check; the kiosk reclaim kept relaunching the Activity; the keep-screen-on retry, the dashboard reveal after an Activity rebuild, plugin screensavers and the permission rows on the settings pages all waited too. All of them now take the app coming back on screen as the return, focus or not, and a restart is no longer needed.
...
### Fixed
- **Kiosks behind an mDNS reflector are listed under their own address.** The kiosk switcher, Fleet Management and the intercom listed a kiosk under the address its announcement arrived from, which through a reflector between VLANs is the router's, so every kiosk on the other VLAN showed the same address and none could be reached. The address the announcement itself carries wins now, the sender only stands in without one.
...
- **An intercom between kiosks.** Kiosks on the same network can call each other and talk to every kiosk at once, with the voice going straight between them over the remote admin port and no server in the path. A new Intercom page on the device and in the remote admin holds the switch, a shared Intercom key (made on first enable, pasted between kiosks or synced by Fleet Management as a credential), the Answer mode (Ring, Answer automatically after one ring and a countdown, or Do not disturb), Ring for, the Ring sound (a built-in telephone ring or a file from the sounds folder), a Show in the kiosk menu switch, the Talk mode (Push to talk by default, Hands free with Mute on devices with an echo canceller) and a Kiosks card that says which kiosk is Ready, off, on another key, on Do not disturb or offline. The kiosk menu gains an Intercom entry with its own Allowed Actions switch, Gestures a Call a kiosk action that rings a chosen kiosk and an Open Call a kiosk action for the sheet, the remote Overview a Do not disturb tile and Home Assistant five ESPHome entities: an Intercom enabled switch, the state, the other kiosk's name, a Do not disturb switch and the answer mode. Announce to all in the kiosk menu talks to every kiosk one way, behind an Accept announcements switch on the receiver. An Intercom volume slider sits under Screen & Audio beside the media and assistant volumes. A call wakes the screen, draws over the screensaver, holds the ambient features and ducks the music like a voice turn and pauses wake word detection. Lockdown Mode answers as Do not disturb, a second caller gets Busy and a missed call is a toast with Call back.

### Fixed
- **A device with no WebView provider no longer restarts forever.** When Android has no WebView package at all, as on some repurposed smart displays, the dashboard can never come up and the frame watchdog answered with a process restart every thirty seconds, for hours, straight through an update. The app now recognizes the missing provider, from Android where it can say and from the error the WebView creation throws elsewhere, puts a notice in the dashboard's place saying what to install, and the watchdog stands down.
...
- **Text scaling slider for the Immich photo details.** The details panel drew its lines at fixed pixel sizes, so what read right on a tablet was too small on a wall display and nothing could change it. A Text scaling slider under Metadata on the Immich Media screensaver page scales the text, icons and spacing from 50% to 200%, previews live and stays out of fleet sync by default like the other scale settings.
- **Screensaver widgets can pick their font family and weight.** The corner overlays always drew in Rubik at one weight. The Widgets group now has Global font family and Global font weight rows with the Clock screensaver's choices, and every widget's editor has its own Font family and Font weight pickers, Default following the global rows, on the device and in the remote admin. Changes apply live to a running screensaver.
...
### Fixed
- **The frame watchdog no longer restarts the app seconds after asking for a WebView rebuild.** The watchdog probes the app every five seconds and restarts the process after six probes that find the app in front with no WebView, twenty seconds after it asked for a rebuild. The probe is a platform call, and the wedge it looks for tends to stall that thread, so on a Lenovo tablet six probes were answered in the same two seconds once the thread came back and the app restarted two seconds after the rebuild it had just requested. Strikes are now paced by the clock and one probe runs at a time, so a rebuild gets its full window before the restart.
- **A Go2RTC candidates entry with a query string no longer sinks every WebRTC stream from that server.** Go2RTC copies the addresses listed under `candidates:` in its configuration into the WebRTC answer without checking them, so an entry written like a TURN URL, `192.168.1.10:8555?transport=tcp`, produces a candidate line with an invalid port. The WebView refuses the whole answer over that one line, the camera fails signaling three times and switches to MSE. Home Assistant's own player never noticed because Go2RTC's socket API hands those candidates over one at a time. The app now drops candidates the parser would refuse before using the answer and writes a warning naming the line and the Go2RTC setting to fix.
- **An MSE stream the device cannot decode now says so instead of sitting black or reconnecting forever.** WebRTC has watched for a stream that connects and decodes nothing since the H.265 work, but MSE logged "playing" as soon as data arrived, so an H.265 stream reaching a device with Allow H.265 streams on and no H.265 decoder either stayed black with nothing in the log or, when the decoder refused it outright, went "playing" then "Reconnecting..." every two seconds for as long as the view stayed up. MSE now runs the same ten second decode watch, treats a media decode error as the codec's fault rather than the network's and only says "playing" once a frame has decoded. The warning names the codec on both transports and, when Allow H.265 streams let the codec through, says to turn it off. A codec that neither transport decodes parks the warning on the tile rather than switching between WebRTC and MSE.
...
- **A slow resume no longer gets the app killed over the Kiosk Satellite Service.** Every resume of the app asked Android to start the service as a foreground service, which comes with a ten second deadline for the service to show its notification. On a tablet where the resume itself stalled the main thread past that deadline, Android killed the app with "did not then call startForeground", seventeen times on two devices. A resumed app is in the foreground already and needs no such start, so the service is now started plainly from there and skipped altogether while it is already up, and the deadline only applies to starts from the background, where it is unavoidable.
...
- **Frame watchdog restarts now say what they found.** The note the watchdog leaves in the crash journal used to be one line. It now names the failure as a WebView that never attached while the app was in front, and adds how many Flutter frames were drawn during the wait, whether a rebuild was tried first, the WebView package and version, the renderer flags, free memory, screen state, uptime and the last browser and watchdog log lines. Crash reports carry the same note, so a device that restarts this way can finally be diagnosed from the report alone. Flutter framework errors and uncaught Dart errors now also land in the app log, capped at twenty a minute, so the Logs screen, the remote admin and that note show why a screen failed to build instead of leaving it to logcat.

### Fixed
- **A radio station no longer shows a full progress bar with matching times.** Music Assistant reports how long a station has been playing, not where the current song is, and when it recognizes the song it attaches that song's duration. The floating player and the Now Playing view read the two together, so the bar sat at the end with the same time on both sides through every song. A radio item now shows no bar and no times on the local player, a followed Music Assistant player and a followed Home Assistant player alike. A Home Assistant player that reports a live channel, or whose position has run past the end of the track it claims to describe, is treated the same way.
...
### Fixed
- **A flapping light sensor no longer floods the Home Assistant recorder.** Some panels ship a light sensor driver that alternates between two far-apart values several times a second with nothing in the room changing. The native damper cannot tell that from a real change, so the Ambient light entity wrote a recorder row every 2 seconds all day. The entity now sits behind a token bucket: a real change, including the settled value the damper sends right after it, still reaches Home Assistant at once, while a sensor that keeps flapping is held to one update every 30 seconds carrying its latest reading. Adaptive brightness reads the sensor directly and is not affected. The [ESPHome guide](docs/esphome.md) notes the limit.

### Changed
- **The Home Assistant tile on the remote admin Overview now says Validated instead of Connected.** The flag behind it records whether this run's connection check passed. It never drops when Home Assistant later goes away, so a green Connected dot could sit under a dashboard pane reporting an outage. The tile and its Needs attention card now describe what the flag actually means: Validated, or Not validated while the kiosk retries the check every 30 seconds.
- **Remote admin cold loads fetch far fewer bytes.** The admin's stylesheet and modules are now minified when the app is built and the embedded server serves the page and every file gzipped to any browser that accepts it, which every browser does. The megabyte of readable source a first visit, or the first visit after an update, used to download is now about a quarter of that on the wire and the browser parses less. The kiosk compresses the bundle once, off the UI thread, when the admin is first opened, so no request costs the panel any compression work. A client that declines gzip gets the minified files as they are. The readable sources moved from `app/assets/remote-ui` to `app/remote-ui` and the [remote API guide](docs/remote-api.md#remote-ui) describes the build step.

## v2026.9.45 - 2026-09-12

### Changed
- **Basic analytics report the device's memory size.** The snapshot carries the installed RAM rounded to the nominal size in half gigabytes, so an Echo Show 8 reads 2 GB and a Tab S8 reads 8 GB. The 2026.9.44 notes listed it, but the field left after that build was tagged.
...
- **Plugins can read and follow Home Assistant entities.** SDK 1 provides an entity picker in both settings interfaces, read-only state and attribute snapshots and selected entity subscriptions through KS-managed authentication. Subscriptions reconnect, report unavailable states and stop with their plugin session.
...
- **Plugin screensavers use the stock screensaver system.** SDK 1 plugins can register rendering documents that appear in the mode picker and schedule editor in both interfaces. KS owns activation, brightness, wake behavior, widgets, At a Glance, pixel shift and Now Playing layouts. Renderers end with their plugin session. Plugin mode selections and schedules remain local to each kiosk.
...
- **Plugins can control hardware and publish RGB lights.** SDK 1 includes verified native libraries, runtime status, grouped settings with color pickers, sliders and dropdowns and namespaced ESPHome RGB entities. The Rockchip LED plugin keeps its driver, effects and root helper in a separate repository.
...
### Changed
- **Plugin settings follow the app's editing pattern.** On-device text settings open an edit dialog. Text, switch, selection and color changes save as they are completed and sliders save when released. Native settings and Remote Admin no longer have a separate Save settings row.
- **Plugin action groups are simpler.** Remove the action assignment guidance below the Actions group in native settings and Remote Admin.
- **Plugin entry controls are easier to use.** The enable switch sits before the plugin name. A check for updates button previews the latest repository release and an info button beside delete opens the saved repository README in a modal. Plugin subpages use a regular description row and contain only plugin settings and actions.
- **Plugin updates no longer require a routine app restart.** KS automatically stops a running plugin, installs its replacement and restores its enabled state while retaining compatible settings. Failed activation restores the previous version and resumes it when shutdown was clean. A timed-out callback or failed stop still requires an app restart before enabling or replacing that plugin.
...
- **Full assistant volume range.** A new toggle below Assistant volume is on by default. It initializes the built-in speaker's call volume to 100% when the native assistant audio route is acquired after startup or re-enabling the setting. Master and assistant volume still control playback. The initialized level also applies to other apps using call audio and is not restored afterward. Later call-volume adjustments are preserved.
- **Optional microphone audio for RTSP streaming.** The new **Include microphone audio** toggle under Camera > RTSP Streaming adds mono AAC audio using the same microphone capture as native wake detection and voice interactions. The toggle is off by default. Audio encoding runs on a separate worker only while a viewer requests audio. Streaming continues when Voice Satellite is muted or Lockdown Mode is on and pauses while the browser owns the microphone. Stream Status shows audio activity and errors on the device and in Remote Admin.
...
### Changed
- **Camera motion controls have their own page.** The former Motion Detection group is now under **Camera > Motion Sensor** in the app and remote admin. It contains the motion sensor switch, clear delay, frame rate, sensitivity and startup delay.
...
### Fixed
- **Scheduled screen-off overrides use one-minute steps in Remote Admin.** The schedule editor's **Turn screen off after** slider now matches the main setting and the device's schedule editor instead of moving in five-minute steps.
- **A failed dashboard no longer blocks Remote Admin.** Voice Satellite controls load independently and JavaScript requests time out when the WebView stops answering. Recovery terminates the failed renderer and releases the old WebView without waiting forever for callbacks. The dashboard watchdog also detects repeated JavaScript timeouts.
...
- **Camera failures include the capture setup in App Logs.** Camera diagnostics now record session IDs, requested and actual resolutions, supported and selected frame rates, encoder startup, first frames and recovery steps. Teardown timeouts identify whether the camera or encoder is still closing. A bounded record preserves the last failure and its recent context across app restarts. Recording runs on a background worker and excludes images and RTSP credentials.
...
- **Fleet Management.** One kiosk leads, the others follow. A new **Fleet Management** page on the kiosk and in the remote admin. The leader adds a kiosk from the ones heard on the network, hands it a profile and sends an invitation that lands as a dialog on that kiosk's screen, where it is accepted or declined: never from the remote admin and with no password involved. A profile is a named collection of settings, credentials and exclusions with a page of its own: the categories to sync, each with what is not synced inside it, the credentials to include one switch each (the Home Assistant token off by default, the Music Assistant token and the Immich API key on), the dashboard and the excluded settings, listed as their full path with the setting's own hint. Sizes, zoom levels, brightness levels, volumes, device-bound files and picks and camera tuning are out of every new profile since they depend on the panel, the speaker or the device and any of them is one switch away from coming back, while any other setting can be added to the list. The built-in Default profile is editable, more can be added, duplicated and deleted and each follower is handed one from its row. From then on the leader pushes what the profile allows to the kiosks on it, whenever something in it changes and whenever a follower reports it drifted and only between kiosks on the same version. What stays per kiosk never travels and is nobody's to change: the device name, remote administration, the ESPHome node name, MAC and encryption key, the Sendspin player id, the followed media player, the assigned satellite, cameras, microphones, renderer workarounds and the UI scale and the follower drops them itself whatever a leader sends. A follower's synced categories wear a banner saying who leads them. **Update the fleet** installs the release offered to each follower behind the leader, followers first and the leader last, with **Keep followers on this version** doing the same on its own. The Overview says when an invitation waits on the kiosk and when followers run another release. A kiosk on a build without Fleet Management shows as such in Add a kiosk. Every fleet call between kiosks rides a token good only for the fleet endpoints and only while the follower lists that leader.
...
### Changed
- **The kiosk menu's header spaces its two lines better.** The device name under Kiosk Satellite sits a step lower and a point larger, and both lines a touch further down the mark.
- **The remote admin's phone top bar shows the device name.** The mark grew a little and the title became two lines, Kiosk Satellite over the device name, so a phone says which kiosk it is looking at without opening the drawer.
...
### Fixed
- **A single tap no longer dismisses the Now Playing view under Double tap to dismiss**. One finger reported twice, the kiosk screen's pointer listener as it landed and the screensaver's gesture detector as it lifted, and a tap held longer than the chain's debounce had its lift counted as the second tap, so a slow tap in the space around the controls closed the view. The lift now reports as the echo of the touch already counted, so the chain counts one tap per finger.
...
- A **volume control on the Now Playing view**: a speaker toggle leads the row of small toggles and a tap swaps the progress bar for a volume slider with the level beside it, back to the progress bar four seconds after the last touch or on a second tap. It sets the device's media volume for this device's own player, the followed player's own volume for a Music Assistant or Home Assistant player and the room's or the group's volume for a Sonos. The speaker beside the slider mutes and unmutes the player: its own mute for a Music Assistant, Home Assistant or Sonos player, and for this device's own player its media volume held at zero until the unmute puts the level back. A player without volume control leaves the toggle out, a blank keeping the transport centered.
- **Sonos speakers can be followed directly**, over the speaker's own local interface, with no Home Assistant or Music Assistant in between. The Sonos page finds the speakers on the tablet's network or takes one by address for a speaker on another VLAN, remembers the household and lists its rooms with a Forget each and the Player dropdown then offers every room as the Sonos app names it, grouped rooms as one entry. Its Enable lyrics switch takes the room's lyrics from Music Assistant and is disabled without that connection, and its Adjust the group volume switch decides whether the volume slider sets the whole group's volume or the room's own while the room plays in a group. A followed room shows its track, artwork and progress read from the speaker once a second, so lyrics stay in step, takes play, pause, stop, next, previous, seek and shuffle, lists the speaker's queue in the Now Playing panel with a tap to jump, follows the group the room plays in as the Sonos app regroups it and stops instead of pausing a radio stream the way the speaker itself does. Artwork for Spotify and Deezer tracks comes from the service's own public image lookup rather than the speaker's art proxy, which fetches the image through the speaker and hangs when the speaker cannot reach the service.
- The floating player and the Now Playing view can follow **any Home Assistant media player**, on the device and in the remote admin alike. A **Player source** dropdown at the top of the renamed **Media Player** settings page picks this device, Home Assistant, Music Assistant or Sonos and a **Player** dropdown under it lists that source's players, every Music Assistant player or every Home Assistant `media_player` entity by name and the pick drives the card, the full-screen view and the transport: track, artwork, progress, play and pause, next and previous, seek and shuffle where the player supports them, with the buttons a player cannot honor left out. Every Home Assistant player is treated the same whatever integration stands behind it. Synced lyrics follow a followed player too, timed from the position its own system reports and looked up on LRCLIB when Music Assistant is not set up and the Now Playing view wears a chip with the followed player's name. A followed Home Assistant player has no queue panel and no favorite heart, since neither exists outside Music Assistant.
- A **Media Player** settings page split into five pages of its own: Sendspin Player (the local player's switch, server, codec, sync offset and voice ducking), Music Assistant (server, token, kiosk menu shortcut), Sonos (the speakers the kiosk knows, a search of the network, an address field for a speaker on another VLAN and the group volume switch), Floating Player (the card's rows) and Now Playing (the full-screen view's rows and the lyrics timing). The Sendspin Player page leaves the settings while another player is followed, as its rows did before.

### Changed
- The **favorite heart** is gone from the Now Playing view. The toggles flanking the transport are volume and shuffle on the left, lyrics and queue on the right, two slots a side.
- The **Music Assistant** settings category is now **Media Player**, with a play glyph; the Music Assistant mark moved to its own page entry. The category keeps its Sendspin id and settings keys, so exports, the remote API and search anchors keep working and a Music Assistant pick stored under the old Player to control setting carries over.
...
## v2026.9.4 - 2026-09-02

### Added
- A **Double tap to dismiss** toggle in the Now Playing group, off by default (#409): a double tap anywhere on the Now Playing view dismisses it and the close button goes away, for the screen everyone is used to tapping. Taps on the transport, the toggles and the queue rows never count, so a quick double press on Next skips twice instead of closing the view.
- A **Font weight** dropdown on the Clock screensaver page, beside the font picker, which is now named **Font Family**, on the device and in the remote admin alike: Default keeps each face's own weight, the digital face light (heavy in Nunito), the flip cards regular and the roller heavy, while Light, Regular, Medium, Bold or Black set one weight for the digits on every face. Rubik and Nunito are variable fonts, so every step is a real weight. Applies live to a clock already on screen.
- An **Inter** option in the Clock screensaver's font picker, a clean grotesque in the style of Helvetica Neue (SIL Open Font License), drawn at its display optical size so the big digits get the tighter cut made for them.

### Changed
- The Now Playing settings group reads in a new order: Launch Now Playing when music starts playing first, then Show media controls, Show in the kiosk menu, the "Now Playing" instead of the screensaver switch, Double tap to dismiss and Dismiss "Now Playing" on motion.

## v2026.9.3 - 2026-09-01

### Added
- The full-screen **Now Playing** view has media controls now, on the device and in the remote admin alike: the Large floating card's previous, play/pause and next buttons under the cover, sized for touch and acting on the whole playback group, over a progress bar with the elapsed and total time that seeks where the server allows it, with a thumb to drag on Music Assistant and the plain progress line elsewhere. Music Assistant sends no fresh progress report after a seek or a queue jump, so the player carries a seek's target itself and otherwise reads the server's own queue time every few seconds, re-basing the moment the two drift apart, and the bar, the floating card and the lyrics stay on the audio. Four smaller toggles flank the transport the way Music Assistant lays them out: a heart that adds the playing track to the Music Assistant favorites or takes it out, and shuffle, on the left; lyrics and queue on the right, each lit while it is the panel, the choice sticking across sessions and the two keeping each other exclusive since they share one slot; the Show lyrics settings row is gone with that, the button being the one place to flip it. The queue is laid out the way Music Assistant's own is, what already played faded above a Now Playing heading, the playing track under it, then an Up next heading with the count of what follows and the rest, opening on the Now Playing heading; a tap on a row jumps the queue there, the row spinning until the device is actually playing it. Shuffle and the queue follow changes made in Music Assistant itself within a moment, over the Music Assistant connection the lyrics already use, since the Sendspin protocol carries no word of them, and a queue cleared there takes the last track off the card and the view instead of leaving it as paused. The transport sits pinned along the bottom of the screen and stays put whatever the layout does, with the cover, the title and the panel rearranging above it, and it also stays put while the cover and the title cross-fade between songs. With controls on screen a tap can no longer dismiss the view, so it carries a close button in the top right corner, the same floating close the Music Assistant page wears, and the back button and the motion setting still work as before. Pausing from the view keeps it up with its play button for as long as the floating card keeps its paused look, instead of dropping to the regular screensaver; dismissing the view ends that, and a pause made while the view is not on screen never holds it. A **Show media controls** toggle, on by default, turns them off for the control-free display the view used to be, where a tap anywhere dismisses it.
- A **Now Playing** switch among the ESPHome entities: on while the full-screen Now Playing view is on screen, turning it on brings the view up the way the kiosk menu entry does, paused with its play button when the music is, and turning it off dismisses it. Listed while the "Now Playing" instead of the screensaver setting is on.
...
- A **Next screensaver** timestamp sensor among the ESPHome entities: when the idle clock will start the screensaver, so an automation can dim the panel or change a status icon ahead of it by triggering on the sensor directly, and a card can count down to it in a template. A single moment that moves on a touch, a dismissal or a timeout change instead of a seconds countdown, which would have written a recorder row every second. Unknown while nothing counts down: the screensaver off or showing, a voice turn, a camera view, hold mode, another app in front. Under a stream of touches it republishes at most once a minute, and always before the moment it last reported passes, so a trigger on it never fires at a moment the device has already moved on from.
- A **Night card color** picker in the Clock screensaver's Night mode group, shown while the Flip style is picked: the color of the flip cards in the dark, near-black by default, since the cards were the one part of the face the Night background left alone and a card color tuned for daylight kept glowing on the night's black wall.
...
### Fixed
- The corner widgets over the Clock screensaver follow its Night mode now: while the room is dark they take the **Night color** in place of their own, instead of staying white next to the dimmed red digits. The Night color's hint says so.
...
### Changed
- The primary accent is the teal of the app icon's keyline now instead of the sage green, which matched nothing in the logo: buttons, switches, sliders, links and focus rings on the device and in the remote admin alike, the keyline deepened a step to a teal that carries text and white ink in both themes. Sage stays as the green of success states and of the settings rail's icon discs, so the four-color accent cycle keeps its green.
...
- A **Night mode** group on the Clock screensaver settings page, on the device and in the remote admin alike (#391): with the switch on, the clock's digits take a color of their own, a muted red by default, whenever the ambient light sensor reads at or below the **Light level** slider (1 to 100 lx, 5 by default), the way a bedside clock stays readable without lighting the room. The **Night color** applies to the digits on every face, the date and the At a Glance row, and the **Night background**, pure black by default, takes the screen behind them so a face tuned for a lit room does not keep glowing at night; everything returns to its normal colors a little above the threshold, so a reading hovering on the line cannot flicker them, and on devices without a light sensor the switch is disabled with the reason.
...
### Fixed
- The ESPHome Update entity's percentage no longer sits at 0 for the whole install (#387): the download progress went onto the wire as a 0 to 1 fraction where the ESPHome protocol wants 0 to 100, so Home Assistant showed at most 0% until the update finished. The MQTT update entity already converted correctly and is unchanged.
...
### Added
- **Home Launcher** (#219): Kiosk Satellite can register as the device's home screen, replacing the stock launcher outright, on its own settings page on the device and in the remote admin. The system then boots straight into the kiosk instead of flashing the old launcher first, every home press returns to the kiosk and Kiosk Mode's **Disable home button** no longer needs screen pinning or its consent dialog, which some devices re-asked on every boot (**Keep screen pinning** opts the pin back in for its native recents and back blocking). Enabling is one tap in a system dialog on Android 10+, a pick in the system's home settings on older releases and fully silent with [device ownership](docs/kiosk.md), including from the remote admin; a home press while the kiosk is already up closes the menu, settings, overlays and screensaver and lands on the dashboard, and back never leaves the kiosk while it is the home screen, the way every launcher ignores it: it closes the menu or steps the page history instead. The Exit entries leave the menu and the remote admin overview while the role is held, since killing the home screen only has the system relaunch it; Restart stays. The safety net is native and independent of the app's own engine: three starts in a row that never draw a frame hand the home role back to the stock launcher automatically, so a broken install can never strand the device, and turning the feature off, the `releaseHomeRole` remote command and uninstalling all restore the previous launcher on their own. Not available on Fire OS, which locks its launcher; see the new [Home Launcher](docs/home-launcher.md) docs.
...
### Changed
- The **People** and **Exclude people** pickers on the Immich screensaver now list the people you have hidden in Immich too, marked **Hidden** under the name with a struck-out eye, on the device and in the remote admin alike (#382). Hiding a person only takes them out of Immich's own listings, and both filters keep working on them, so a hidden face you never want on the frame can be excluded, and a hidden person you do want can be shown, without unhiding them in Immich first. A person hidden after being picked also no longer drops out of the filter the next time the row is saved. Unnamed face clusters are still left out, hidden or not, since the filters pick by name.
- **Fill the screen** is now a choice of Off, Smart or Always in all four photo screensavers, Home Assistant Media, Local Media, Photo Gallery and Immich Media, on the device and in the remote admin alike (#378). The switch only ever cropped a photo already shaped close to the panel, about a quarter along one axis, and framed everything else over a blurred backdrop, which leaves a 4:3 camera photo on a 2:1 panel such as an Echo Show 5 sitting between bars with no way to overrule it. **Always** crops every photo to the edges whatever its shape, cutting off whatever does not fit, and there is no knowing what the crop takes: a subject that is not centered is the thing most likely to go. **Smart** is what the switch on did, and an existing install keeps its behavior, on becoming Smart and off becoming Off.
- The docs no longer offer device ownership as something an Amazon Fire tablet can be talked into. Fire OS provisions Amazon's Parental Controls as profile owner on the first boot, Android allows one owner per user, and `dpm set-device-owner` refuses whatever the accounts say. [Amazon Fire tablets](docs/fire.md), [Updates](docs/updates.md) and [Kiosk and Lockdown](docs/kiosk.md) all said the accounts were the only obstacle. [Updates](docs/updates.md) now has an Amazon Fire tablets section next to the Meta Portal one, saying outright that a Fire update is never hands-free and what the one-tap path looks like.

## v2026.8.104 - 2026-08-30

### Added
- A **Device name** field on the setup wizard's first page, on the device and in the remote admin alike, above Remote administration and filled in with the device model. It is the name Home Assistant, the remote admin and the network use for the kiosk, and the ESPHome node name is built from it at the ESPHome server's first start, under a `ks-` prefix so every kiosk sorts together, so a Fire HD 10 left at its model name shows up as `ks-amazon-kftuwi` and a kiosk named Kitchen Tablet as `ks-kitchen-tablet` with `esphome.ks_kitchen_tablet_notification` actions, instead of a generated `kiosk-satellite-<id>`. A name typed in the ESPHome Node name setting itself is still taken as written. It can be changed any time under Settings, Device.

### Fixed
- Video pushed over DLNA no longer fails with an error icon on MediaTek devices (#374), among them the Echo Show 8 and the Lenovo Smart Tab M10. The video surface Flutter draws into holds seven frames of its own, so the decoder was asked for fourteen buffers where it caps out around ten, and it refused to start: the file, the codec and the profile were never the problem, and the same video played in a dashboard card. Such a video is now retried once on a platform view, which asks for far fewer buffers, and it plays. Screensaver videos, local and Immich, take the same retry.
...
### Fixed
- Screen on no longer strands the kiosk behind the lock screen (#372). Screen off is Android's device-admin lock, which arms the lock screen on any device that has one, PIN or not, and a panel that woke onto it had the kiosk paused underneath: the screensaver stood down, its wake hold went with it and the lock screen's own timeout put the panel back to sleep about fifteen seconds later, over and over, so a Fire HD 8 switched on from Home Assistant or by the screensaver's Turn screen off after timer never stayed on. Every wake now clears a lock screen with no PIN, pattern or password behind it, through the same activity that lights the panel where the wake lock does not (on Android 10+ that takes the Display over other apps permission, already requested by the wizard). A secured lock screen is never fought: the kiosk waits behind it until the device is unlocked and the log says so. Fire OS 7 lets it through, verified on a Fire HD 8. Fire OS 8 refuses the dismissal to every app but Amazon's own, so there the kiosk stays behind the lock screen as before, and the log now says that too and names the fix, `adb shell locksettings set-disabled true` followed by a reboot. A new [Amazon Fire tablets](docs/fire.md) doc covers the lock screen, the switch and everything else that is different on a Fire, the way the Meta Portal doc does for a Portal.
...
### Changed
- The [ESPHome](docs/esphome.md) doc is rewritten lean: the same facts in tables and lists, a third shorter, with the setup, the connection budget, the entity notes, the notification arguments and sounds, renames and the identity switch each in a table of their own.

### Added
- Person detection from a sensor the device itself runs, on Meta Portal devices today. Portal OS runs a person detector all the time for the Smart Camera's auto-framing, on a virtual camera feed that never lights the camera LED. It logs a heartbeat every 30 seconds while someone is in view. A new **Settings, Screensaver, Person Detection** page, which only exists on a device with such a sensor, offers **Dismiss on person** and **Postpone screensaver on person**, Motion Detection's shape on that sensor: no camera session of the app's, no face model, people at any angle rather than faces and independent of the camera legs. Presence is a state, so while someone is in view the postpone leg holds the idle clock continuously. On the Portal the Smart Camera's framing director is read too, so a person moving registers within seconds rather than at the next heartbeat. An **Occupancy** row under the switch shows what the sensor reports and the age of the last heartbeat. With Dismiss on person on, the ESPHome device carries a **Person** occupancy binary sensor with the same state, re-registering the device when the switch flips. A Required system permissions group at the foot of the page shows the one grant the Portal's sensor needs, Log access, which Android gives only over adb and only to a restarted app: the row says so and points to the doc for the command (the remote admin also offers it with a copy box), then offers a Restart button once it is granted. The [Meta Portal](docs/portal.md) doc covers the grant, the sensor's latency and what else is different on a Portal: the DNS server Meta adds that bypasses split-horizon DNS, the hidden service notification and the accessibility list to append to rather than replace.
- The kiosk's GPS position as ESPHome sensors (#363), for a tablet that travels: GPS latitude, GPS longitude, GPS accuracy, altitude, speed and the time of the last fix, so an RV's tablet can drive zones, time zone changes and border-crossing automations from the receiver that is always in the vehicle. Opt-in on the new **Settings, ESPHome, GPS Sensor** page, on the device and in the remote admin alike, with the update interval, the last coordinates under the switch and the one grant the receiver needs in a Required system permissions group. Turning the switch on or off re-registers the ESPHome device. On a device without a GPS receiver the switch is disabled with the reason. Readings keep arriving with the screen off: the Kiosk Satellite Service attaches Android's location foreground type while the switch is on.
...
- On a phone, a settings page's title sits right after the back arrow instead of a gap further along, the way the tablet's settings pane already places it.
...
## v2026.8.94 - 2026-08-28

### Fixed
- The Screenshot camera is back on every ESPHome device, beside the device camera where there is one. The screen capture (the frame the remote admin's preview shows, fed by the Take screenshot button and by a fetch from Home Assistant) used to share one camera entity with the device camera, on the belief that ESPHome allowed one camera per device, so a kiosk with a camera only got the Screenshot camera while the camera was switched off, and v2026.8.93 took that away too. The limit was the ESPHome firmware's, not the protocol's: Home Assistant assembles camera frames per entity, so both cameras now exist at once and a fetch of either refreshes both. Last screenshot moves when the Take screenshot button captures and when the remote admin's overview page fetches its preview (which now also refreshes the Screenshot entity over ESPHome and MQTT), never on a fetch of the camera entity, and it and Last camera snapshot keep their stamps across restarts instead of reading unknown until the next capture.

### Added
- Adaptive brightness (#343): the kiosk dims its screen from its own ambient light sensor, on the device, so the mapping keeps working with Home Assistant or the network away. It is a page of its own under Screen & Audio: the switch, the sensor's live reading, Minimum and Maximum brightness, and the Dark room and Bright room light levels the curve runs between, typed rather than slid because sensors disagree wildly about what a lit room reads (an Echo Show 8 says about 50 lx with every light on), and checked against each other so Dark room stays below Bright room, as Minimum and Maximum brightness are. The brightness follows the log of the reading between the two, so an evening living room sits halfway rather than at the floor. While the switch is on, Default brightness stands down and a session starts at Maximum brightness dimmed for the room; the screensaver's brightness and the Dim level keep their meaning as the level in a bright room and dim by the same share. The switch is offered only on devices with a light sensor, and where it exists it is also an Adaptive brightness switch entity on the ESPHome and MQTT devices.
- A Panel brightness diagnostic sensor on the ESPHome and MQTT devices reports what the panel actually shows, in percent, following every step of the adaptive curve.
...
- Fields match on both surfaces. A time of day (the theme times, a schedule entry) shows in a control box with a clock glyph and opens one time picker dialog on the device and in the remote admin: hour and minute boxes with stepper chevrons (hours by 1, minutes by 5) that can be typed into, in place of the Material dial on the device and the browser's own time input on the remote. A color shows as the 34 swatch on both and opens the same picker dialog (live preview, three channel sliders, the same 8 presets at 34) on the remote as on the device. The ESPHome encryption key is a read-only copy box on both: tapping it copies the whole key, the disc flips to a check and a toast says Copied. The remote's switches, checkboxes and radios are drawn at the device's Material 3 sizes (the off thumb sits centered in its ring), its fields are the device's 44 high control box with the primary focus border, its dropdowns carry the chevron on the bordered box, and its sliders run the full row width under the name and value the way the device's do, on the settings pages, the Overview brightness, the master volume and the Voice Satellite sliders alike.
- Messages and dialogs match on both surfaces. One banner shape carries a state that makes a section inert or a failure that blocks a page: the Bluetooth off notice on the ESPHome page and the setup wizard's connection errors are tinted containers with an icon on the device and in the remote admin alike. The remote admin has the device's toast now, a compact card at the bottom of the page: a quick control that lands says so, a copied log or key says Copied, and the message box stays for answers that need a button. The remote's hint rows under a setting carry the device's info or warning icon, its message box body reads at 15, and labeled fields in the editors of both surfaces stack 16 apart.
- Lists and navigation match on both surfaces. Orderable lists (the cameras in a view, the camera views a screensaver cycles, the At a Glance entities) carry the drag handle plus Move up, Move down and Remove on the device now, the way the remote admin's do, so a tablet reorders without a drag, and tapping a glance entity's row edits it. The schedule and widgets editors end in an add row (Add time, Add widget) on both surfaces in place of a button, and an add row with no corner left stays put, disabled. The remote's rail tiles take the device's size and radius, its external page rows use the trash icon action, and a segmented pill that does not fit a phone (the Logs source, the File Manager location, the import choice) scrolls sideways under the edge fade on both surfaces instead of wrapping or overflowing, down to a 320 wide phone.
- Editor fields carry their label above the control on both surfaces, in the quiet 12.5 text, 6 apart. The device's dialogs (cameras, servers, views, gestures, schedule entries, widgets, glance entities, setup) drop Material's floating label that sat across the field's top border, and the remote admin's editors draw every label at that size and keep the label's spacing when a field is shown again after its trigger type changes (the Gesture and Fingers labels in Edit gesture, and the Edit camera fields, sat at different distances from their controls).
- The remote admin's File Manager no longer rebuilds the page on every click. The root tabs, the upload button and the path header stay put, and the listing is a fixed pane that reaches the bottom of the window and scrolls inside under the edge fade, the way the Logs page works, so opening a folder, going up or switching between the app folder and shared storage swaps the rows and moves nothing else. Deleting a file asks in the app's own dialog, and upload or download failures say so in a toast.

### Fixed
- The device's settings search box is the same pill as the remote admin's: it was drawing at the control radius with a hairline because the theme's field borders outranked its own shape.
- The remote admin's icon actions (Up one folder, download, delete, move, play) hover as the round 40 disc they are drawn in, not a narrow oval: the shared button builder was forcing an automatic width onto them.
...
### Changed
- The settings line names its caller in brackets the same way, `set screensaver.enabled = false [esphome]`, in place of the `via esphome` form of the last release, and the remote admin page is `[remote admin]` on both lines.
...
- **Show fingers** as a gesture: a hand showing one to four fingers, or an open hand, to the camera, each its own trigger, so five actions can live on one hand. MediaPipe's hand landmarker finds the hand, tracks it and reads its 21 joints, from which the raised fingers are counted; the first look that reads the configured count fires the action, and the count has to change, or the hand go, before the same mapping fires again. A finger is up when its tip is farther from the wrist than its middle knuckle; the thumb is not counted, so the counts are the four fingers and an open hand is all four. The hand landmarks come from MediaPipe's own hand landmarker task (palm detection, tracking and landmark smoothing in one graph), with its native libraries kept out of code shrinking, which they need. It runs on the device, only when the picture changed or a hand is in view; nothing is identified, stored or compared.
...
### Added
- The Camera Streams screensaver cycles through several camera views. The mode's single **Camera view** row is now a **Camera Streams screensaver** page, on the device and in the remote admin alike, with a **Camera views** list that takes any number of the views configured under Camera Streams, in the order they show (dragged into place on the device, moved a step at a time in the browser), and **Seconds per camera view** for how long each stays up, 30 by default; with a single view selected nothing rotates. The seconds count from the moment the view shows video, not from when its streams start negotiating, so a slow camera does not eat into its own time on screen; a view whose cameras never come up moves on after its time plus a 20 second grace. A rotation never plays two views at once: the grid on screen is shut down first, its streams stopped from the page and given the same grace an opened camera view gets on close, and only then is the next view mounted, so a tablet with little memory sees one set of peer connections and decoders at a time and nothing is left running behind the screensaver. That hand-off costs a moment of black between two views, and the dwell has a floor of 5 seconds so a rotation cannot turn into a permanent reconnect. The rotation also holds while the app is behind another app or a dark panel, since a grid nobody sees is not worth restarting. Editing the list or the seconds while the screensaver is showing applies at once, the way the schedule editor does, so a kiosk left on its black cover with nothing selected shows the views the moment they are picked. The view chosen before the update becomes the whole rotation on first start, and an older backup's single view does the same when imported. The **Screensaver next slide** and **Screensaver previous slide** buttons on the ESPHome device, and the `nextScreensaverSlide` and `previousScreensaverSlide` remote commands, step the rotation by one view either way, through the same hand-off, and the view they land on holds for its full time. A **Mute all views** toggle on the same page, on by default, keeps the screensaver silent even where **Play sound for a single camera** under Cameras would let a one-camera view play its sound, since a screensaver left up overnight has nobody to talk to; a view opened by hand keeps following that switch. Muted views also fetch no audio track at all.

### Changed
- The **WebRTC Camera** screensaver mode is now **Camera Streams**, the name of the settings page whose views it shows, since those views have played WebRTC, MSE, HLS and MJPEG streams alike for a while. The stored value is unchanged, so a schedule entry, an automation or a backup naming the mode keeps working; the Screensaver mode select on the ESPHome device offers the new label, so an automation that selected the option by its old label needs the new one.
...
### Fixed
- The brightness never came back after a screensaver that dimmed to 0%. On a panel whose lowest setting is above zero (the Galaxy Tab S8's scale runs 2 to 255) the app wrote a plain 0 for 0%, which the display can only show as its floor, and Android 14's brightness synchronizer then kept 0 as the value it preferred while the panel reported the floor; from that moment it judged every later write, the screensaver's restore included, as a conflict and put the floor back, and the tablet stayed at its darkest until adaptive brightness was toggled by hand. The app never writes under the panel's floor now, 0% included (it is the floor on such a panel, which is as dark as it goes), and every brightness write is checked again once the system has had its say: one that was reverted gets the adaptive brightness toggle that clears the synchronizer and is written once more, with a log line saying so, which also unwedges a tablet already left that way by an older build.
...
### Added
- Home Assistant can take notifications down again (issue #321). The `notification` action now answers with the kiosk's id for the card, `{"id": 7}`, read in an automation through `response_variable` (ESPHome action responses, supported by Home Assistant since 2026.1), and a second action, `esphome.<node>_notification_dismiss`, takes that id down again, or clears the screen when given `0`. A door-open card can now come down the moment the door closes without touching another door's card, and a leak alert can be cleared everywhere once the condition clears. A **Notifications dismiss all** button on the ESPHome device is the same clear as a dashboard tile.
- Notifications pushed from Home Assistant can bring their own sound and volume (issue #320). Every sound comes from one folder on the kiosk, `Android/data/me.jxl.kiosk_satellite/files/sounds`, the app folder the File Manager shows; files get there through the new **Add a sound** row on the settings page (Browse on the device, Upload in the remote admin), the File Manager, USB or adb, and MP3, OGG, WAV, FLAC, M4A and AAC are accepted, the formats every supported Android decodes natively, so a video file is refused. The `notification` ESPHome action and the `showNotification` remote command gain `chime_file`, the name of a file in that folder to play in place of the default, so a leak alert and a laundry-done can sound different across a house full of kiosks; a name with no file behind it plays the default with a line in the log saying so. They also gain `volume`, 0 to 1, which stands apart from the media and assistant faders, since a notification is neither; 0 means the setting. Those defaults live on a new **Notifications** page under the ESPHome settings, above Bluetooth Proxy, on the device and in the remote admin alike: **Notification sound** is a dropdown over the folder, Built-in chime first, storing the file's name so a restored backup keeps meaning the same file; **Notification volume** sets the level, 70% by default; and the page names the exact action for this kiosk with a **Test** button that sends a notification through it.
- **Screensaver next slide** and **Screensaver previous slide** buttons on the ESPHome device. They step a showing Home Assistant Media, Local Media, Photo Gallery or Immich Media slideshow by one, forward or back, and the new slide holds for its full interval instead of what was left of the old one; a video stepped past no longer ends the slide that replaced it. With any other mode up, or no screensaver at all, a press does nothing, so an automation or a dashboard button can send them at any time. The remote API carries the same two as `nextScreensaverSlide` and `previousScreensaverSlide`.
- The App Launcher's **Return automatically** now waits for the other app to go idle (issue #317). **Return after** used to be a countdown from the moment the app opened, so someone in the middle of a recipe or a call was pulled back to the kiosk regardless; it now counts the time the other app goes untouched, and every touch in it starts the count over. Android has no way to ask when the screen was last touched, so while the clock runs the app keeps a one-pixel, invisible, untouchable window over the other app that Android tells about every touch that lands elsewhere, which rides the same Display over other apps grant the return itself needs. A **Required system permissions** group appears under the switch while it is on, on the device and in the remote admin alike, listing that grant and Unrestricted battery with what each one is for and a button to give it. An app that hides overlay windows, as some banking apps do, keeps its touches to itself; there the clock runs plain and the kiosk comes back after the time as before, with a log line saying so.
...
### Added
- **Kiosk Satellite Service**, one foreground service that keeps the app alive on every install. Android freezes a backgrounded process whole and the battery managers some manufacturers add kill it outright, so a few hours into the night the Home Assistant session, MQTT and everything else stopped together and the kiosk read unavailable until someone touched it. Until now only two features carried the exemption that prevents this, background listening and the ESPHome server, each with a service of its own, and the common install with neither had none. The app now runs a single service from its first start, whatever is switched on, with one notification whose text says what it is doing. The features only add to what it declares: background listening adds the microphone, an enabled camera adds the camera, so motion detection keeps its camera through screen-off without background listening on, the Bluetooth proxy adds Bluetooth scanning, and MQTT, the ESPHome server, remote administration and the kiosk protections are listed as reasons. The service also holds the Wi-Fi lock for as long as it runs and, new, a CPU wake lock while the panel is dark, so the timers behind the keepalives fire on time instead of waiting for the next interrupt; that lock is the service's one setting, **Keep the CPU awake while the screen is off**, on by default, for tablets on battery to turn off. A new **Kiosk Satellite Service** page under Settings, Device, on the device and in the remote admin alike, shows whether it is running and holds its foreground exemption, the types it declares, its locks and notification, what it is keeping the process alive for, and the grants that matter for that with a button to give each one. The relaunch after a crash or a close from recents rides on the same service. The setup wizard introduces it on its first page, right under Restore from configuration file, on the device and in the remote admin alike: what it is, and its three grants with a button for each, and the wizard's Permissions page now asks for the notification grant on every install rather than only with background listening. The two wizards now open on the same first page: the remote one used to skip its Welcome step whenever the admin password already existed and put the restore path on the Connect step instead, so the two differed in step count and layout; it now always starts with Welcome (the password only where one is still needed), and the Connect step is the Home Assistant connection alone on both. Remote administration is on by default on the device wizard's first page, whose three groups now carry titles (Remote administration, Restore backup, Required Service Permissions), and the page cannot be left while a grant the service needs is still missing (Unrestricted battery, Display over other apps and Notifications): Next names the grant in the way instead.
...
### Fixed
- The ESPHome encryption key gains a **Copy** button next to its field in the remote admin. Double-clicking the key selected only part of it, since a double-click stops at the `+`, `/` and `=` characters a base64 key is full of, and a partial key pasted into Home Assistant fails with a generic "unable to connect". The button copies the whole key, and works on the plain-http address the admin is usually opened on, where the browser's clipboard API is not available.
- The remote admin's setup wizard failed its own first step. Setting the admin password restarted the remote server, under the very request that set it, so the browser saw a failed fetch and stayed on the password step, and pressing Next again was refused with "setup already done" because the password had in fact been saved. The server now restarts only when its port changes; a password set or changed while it serves takes effect in place. Should a password already exist when the wizard asks for one (set on the tablet's own wizard meanwhile), the remote wizard now says so and hands over to the login page instead of an error. Going back to the Welcome step also lets the password be changed on both wizards: the remote one always shows the field (empty keeps the current password, and a change rides the wizard's own session, so nobody else on the network can take an unconfigured tablet over), and the device one starts the field with the password already set instead of refusing an empty box.
- A dashboard that stayed dead after a screensaver was dismissed. The screensaver freeze hides the dashboard's native view to stop it compositing under the overlay, and the app keeps its own note of whether it did. The two could fall out of step, with the view hidden while the app believed it was not, after a freeze the app judged had not taken (the dashboard reported as not found mid-rebuild or mid-navigation), and from then on every dismissal skipped the reveal because there was nothing to reveal by the app's books: the page kept running, connected and answering, behind a view that never came back. The native side now reports the views it matched rather than the ones it changed, so a view already hidden counts as frozen and thaws normally, and the edge from a screensaver that covered the dashboard to one that does not always asks for the reveal, which costs nothing on a view that is already visible.

### Changed
- The "Listening for a wake word" and "ESPHome active" notifications are gone, folded into the Kiosk Satellite Service's one notification, and the two notification channels behind them are removed on update. The ESPHome server's unconditional CPU wake lock is now the service's, under the setting above.
...
### Added
- Face detection for the screensaver. A new **Dismiss on face** switch, on its own Face Detection page under Screensaver, wakes the screen only when someone looks at the kiosk: a person walking through the room leaves the screensaver up, and the dashboard appears when they turn to the screen, the feature Fully Kiosk calls face detection. It is detection, not recognition: a small on-device model notices that a face is looking at the camera and nothing more, nothing is identified, stored or sent anywhere, and the camera runs only during the screensaver, exactly like Dismiss on motion. A **Face sensitivity** slider sets how close the face has to be, from right at the screen to as far as the camera can make one out, and **Postpone screensaver on face** extends it the way the motion one does: someone looking at the kiosk keeps resetting the idle timer, at the cost of the camera and the face detector running permanently. The detector is built for the low-powered devices most kiosks are: the motion analyzer's grid decides when there is anything to look at, so the model runs only while something moved, a face was recently in view or the camera just opened, at most twice a second on a single core and slower where a run costs more, so it never takes more than about a fifth of a core, and an empty room costs no inference at all. Two rules apply: Dismiss on motion takes precedence, so with both on the motion switch owns the wake-up and the page says so under the face switch; and faces need light, since a face detector cannot see in the dark the way the motion detector can. For that, the screensaver schedule gains a **Dismiss on face** override next to the motion one, so a daytime entry can wake on faces while an evening entry switches to motion for the dark hours. The switch is exposed to Home Assistant as **Screensaver face detection** over MQTT and ESPHome, the remote admin's Device page reports whether face detection is active, and pages get a `kiosksatellite:face` event and a `getFaceEnabled()` read next to the motion ones.
- The Wi-Fi MAC address can be typed in where Android will not reveal it. **Use real Wi-Fi MAC address** only works where the app can read the address, Android 9 and older or a device-owner install, and everywhere else the switch could only say so and leave the generated identity in place, keeping the kiosk apart from the entries the router and network integrations hold for the same hardware. The status row under the switch now offers a **Spoof Wi-Fi MAC address** field whenever the hardware read fails: type in the address from Android's About > Status page or the router's client list and the kiosk reports it to Home Assistant over ESPHome and MQTT alike, the same as a read one, so the device entries merge as intended. The field accepts the usual spellings and refuses anything that could not be an interface address, a working hardware read always wins over a typed one, and the same re-add caveat as the switch applies: a new address is a new device to Home Assistant. Both rows live on a new **Advanced settings** page at the end of the ESPHome settings.

### Fixed
- The hardware detail the MQTT sensors carry as attributes now reaches Home Assistant over ESPHome too. The Device, IPv4 address and IPv6 address sensors exist on both integrations, but the ESPHome protocol has no attributes at all, so over ESPHome the Android version, the OEM build and the per-interface address lists simply were not there. They are now entities of their own: **Android version** and **Android build** next to **Device**, and **IPv4 addresses by interface** and **IPv6 addresses by interface** next to the address sensors, each reading `wlan0: 192.168.1.5; eth0: 10.0.3.2` so an automation can tell whether the kiosk is on its wired or its wireless NIC. The ESPHome address sensors also gained the two habits their MQTT twins already had: a routable IPv6 address leads over the link-local `fe80::` one, and both are re-checked moments after a network change instead of waiting for the minute poll.
- Old GPUs that crash the moment the dashboard appears now draw it another way. On Android 9 and older the app hands the dashboard to Android and takes its own frames back through a buffer that asks for one pixel format without being able to tell the driver what to produce, so a GPU that answers in a different one (the Mali in the 2014 Galaxy Note 10.1) makes that read fail and takes the whole app down. The result was a launch loop that started right after the setup wizard, seconds into every launch, with no way back into the app. The app now puts the question to the GPU once, before it draws anything: it repeats the exact frame the renderer would draw, reads it back the same way, and a driver that refuses gets the dashboard drawn into a texture instead, which every GPU handles. Hardware the native path works on is untouched, since that path is the faster of the two, and the choice is visible and reversible as a new **Legacy WebView renderer** switch in the Device settings, next to Legacy renderer; it takes effect on the next app start.
- **Screen on** now wakes panels that ignored it. The command had one route, a wake lock flagged to wake the device, which some devices accept and then leave the display dark (reported on a Galaxy Tab S7+), while the app logged "screen on" over a black panel and every mirror of the screen state, the remote admin's tile and the Home Assistant entities, said lit. The app now looks at the panel a moment after the wake lock and, when it stayed dark, wakes it the way alarm and call screens do, through an Activity the system lights the panel for; that route needs the Display over other apps permission on Android 10 and newer. Both attempts are logged, so a device where neither works says so in the Logs instead of claiming success, and the reported screen state follows what the panel actually did. To tell the two routes apart on such a device, `screenOn` over the remote API accepts `{"path": "activity"}` to try the second one on its own.
- The status row under **Use real Wi-Fi MAC address** in the remote admin UI appears the moment the switch is flipped, instead of after a page reload, and now goes away when it is flipped off.
- A settings value refused by its check now says why. On the device the edit dialog stays open and shows the reason under the field; in the remote admin UI the reason appears under the row and the control snaps back to the value the device kept. Before, both quietly kept the old value while looking saved.
...
### Changed
- At a Glance is redesigned to work on every screensaver. Each entity now draws as a chip, a rounded pill in one translucent dark tone sized to its content, with the icon in its own circle leading the name and the state bold under it, so the row carries its own contrast and stays readable over photos. The icon's circle follows the entity's state the way Home Assistant lights up an active tile, a lit light glows amber and an unlocked lock warns red while everything idle stays grey, with a **Monochromatic icons** toggle to keep every circle neutral. With that, the row is no longer limited to the Black and Clock screensavers: it appears on every mode except the WebRTC Camera grid, pinned near the bottom on the photo and web modes, and on the Immich screensaver it wraps into a narrower block whenever the metadata panels occupy the bottom corners, staying clear of them the same way it already wraps on a portrait screen. A new **Floating text style** toggle restores the original floating-text look, and a **Row scaling** slider sizes the row to the screen the way Widget scaling sizes the corner widgets.
...
### Fixed
- The Screen light can turn the display off while Lockdown Mode is on. Turning the screen off pauses the app the same way losing the foreground does, and lockdown's foreground watchdog answered it the way it answers an escape: by pulling the app back to the front, which begins with waking the screen. So the panel went dark for about two seconds and lit right back up, on every attempt, while the very mode meant to keep hands off the device was the one keeping the screen on. The watchdog now checks the panel first and holds its fire while the screen is off, since a dark screen is not an escape, and rearms the moment the screen comes back on, so an app raised over the kiosk while the panel was dark is still answered as soon as there is a lit screen to reclaim. The same held for plain Kiosk Mode with Disable home button on but the pin not holding, where the OS screen timeout or a remote screen-off got the same unwanted wake.
