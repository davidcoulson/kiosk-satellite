...
### Added
- **Provisioning intents are gated.** `ks.provision` configures a kiosk without touching its screen, and its payload is the same JSON the settings import takes — the admin password included. Any app on the device can send an intent, so it is now accepted only while the kiosk has no Start page yet (the out-of-the-box case the feature exists for, and what `tool/onboard-panel.sh` uses), or when the new **Allow provisioning intents** switch is on for an MDM that re-provisions in place. A refused payload is logged rather than swallowed.
